...
type ProductInfoProps = {
  ...
  onSelectOption: (option: ProductPage["options"][number]) => void;
};
...
function ProductInfo({
  ...
  const visible = showMore
    ...
    : product.productFilterOptions.slice(0, 4);
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium text-xs sm:text-sm text-gray-600">
                {item.filterOption.filter.name}
              </span>
            </div>

            <span className="font-medium text-xs sm:text-sm">
              {item.filterOption.name}
            </span>
          </div>
        ))}

        {!showMore && product.productFilterOptions.length > 4 && (
          <button
